...
fetch_records <- c('section_h', 'section_h1', 'section_b', 'section_a', 'section_e', 'section_f', 'section_p')
...
validation_section_h <- as_tibble(ls(pattern = '^cv_h')) %>% 
  mutate(cases = map(.$value, function(x) { eval(as.name(x))[[2]] })) %>% 
  unnest(cases) 

save(validation_section_h, file = '../data/validation/validation_section_h.Rdata')
