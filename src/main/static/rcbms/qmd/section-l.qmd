...
fetch_records <- c('section_a', 'section_l', 'section_l1')
...
validation_section_l <- as_tibble(ls(pattern = '^cv_l')) %>% 
  mutate(cases = map(.$value, function(x) { eval(as.name(x))[[2]] })) %>% 
  unnest(cases) 

save(validation_section_l, file = '../data/validation/validation_section_l.Rdata')
