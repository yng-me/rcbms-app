...
fetch_records <- c('section_o', 'section_o1')
...
validation_section_o <- as_tibble(ls(pattern = '^cv_o')) %>% 
  mutate(cases = map(.$value, function(x) { eval(as.name(x))[[2]] })) %>% 
  unnest(cases) 
  
save(validation_section_o, file = '../data/validation/validation_section_o.Rdata')
