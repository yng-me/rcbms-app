...
```{r, echo=F, warning=F, error=F}
g12_a_check <- section_g11_19 %>% 
  filter(f02_dfarming == 1 | f03_crop == 1) %>%
  mutate(g12a_agrimain_grwcrps = if_else(g12a_agrimain_grwcrps == 2, 0L, as.integer(g12a_agrimain_grwcrps))) %>% 
  select(1:9, f02_dfarming_fct, f03_crop_fct, g12a_agrimain_grwcrps, g12a_agrimain_grwcrps_fct) %>% 
  group_by(case_id) %>% 
  summarise(s = sum(g12a_agrimain_grwcrps, na.rm = T)) %>% 
  filter(s == 0)

(
  cv_g12a_agrimain_noanswer1 <- section_g11_19 %>% 
    inner_join(g12_a_check, by = 'case_id') %>%
...
```{r, echo=F, warning=F, error=F}
g12_b_check <- section_g11_19 %>% 
  filter(f02_eraising == 1 | f03_livestock == 1) %>% 
  mutate(g12b_agrimain_lvstckpltry = if_else(g12b_agrimain_lvstckpltry == 2, 0L, as.integer(g12b_agrimain_lvstckpltry))) %>% 
  select(1:9, f02_dfarming_fct, f03_crop_fct, g12b_agrimain_lvstckpltry, g12b_agrimain_lvstckpltry_fct) %>% 
  group_by(case_id) %>% 
  summarise(s = sum(g12b_agrimain_lvstckpltry, na.rm = T)) %>% 
  filter(s == 0)

(
  cv_g12b_agrimain_noanswer1 <- section_g11_19 %>% 
    inner_join(g12_b_check, by = 'case_id') %>%
    mutate_at(vars(matches('^f0[2-3].*_fct$')), toupper) %>% 
    select_cv(
      f02_eraising_fct, 
      f03_livestock_fct, 
...
```{r, echo=F, warning=F, error=F}
g12_c_check <- section_g11_19 %>% 
  filter(f02_afishing == 1 | f03_fish == 1) %>%
  mutate_at(vars(matches('g12[c-e].*'), -matches('_fct$')), ~ if_else(. == 2, 0L, as.integer(.))) %>% 
  select(1:9, f02_afishing, f03_fish, matches('g12[c-e].*')) %>% 
  group_by(case_id) %>% 
  summarise_at(vars(matches('g12[c-e].*'), -matches('_fct$')), ~ sum(., na.rm = T)) %>% 
  adorn_totals('col') %>% 
   filter(Total == 0) %>% 
   tibble()
...
(
  cv_g12c_agrimain_noanswer1 <- section_g11_19 %>% 
    inner_join(g12_c_check, by = 'case_id') %>%
    select_cv(
      f02_afishing_fct, 
      f03_fish_fct, 
...
### G13 - Not applicable since G12A-B are all 'No'

G13 should be blank since answers in G13A to G13B are all 'No'.

```{r, echo=F, warning=F, error=F}
(
  cv_g14_prodact_na <- section_g11_19 %>% 
    filter_at(vars(matches('^g12[a-b]_.*'), -matches('_fct$')), all_vars(. == 2)) %>% 
    filter(!is.na(g13_typengage_grownlvstck)) %>% 
    distinct() %>%
    select_cv(
      matches('^g12[a-b]_.*_fct$'),
      g13_typengage_grownlvstck_fct
    )
)[[1]]
```
...
### G14A-B - Not applicable since G12A-B are all 'No'

If answered all 'No' in G12A-B, G14A-B must be blank.

```{r, echo=F, warning=F, error=F}
(
  cv_g14_prodact_na <- section_g11_19 %>% 
    filter_at(vars(matches('^g12[a-b]_.*'), -matches('_fct$')), all_vars(. == 2)) %>% 
    filter_at(vars(matches('^g14[a-b]_.*'), -matches('_fct$')), any_vars(!is.na(.))) %>% 
    select_cv(
      matches('^g12[a-b]_.*_fct$'),
      matches('^g14[a-b]_.*_fct$')
    )
)[[1]]
```
