...
(
  cv_r13a  <- section_r %>% 
    ...
    filter_at(vars(matches('^r13[a-ez]_.*'), -matches("fct$")), any_vars(!(. %in% c(1, 2)))) %>% 
    select_cv(
      r12_elctrcty,
      matches('^r13[a-ez]_.*'), 
      h = c(
...
(
  cv_r13a  <- section_r %>% 
    filter(r12_elctrcty == 1) %>% 
    filter_at(vars(matches('^r13[a-ez]_.*'), -matches("fct$")), all_vars(. == 2)) %>% 
    select_cv(
      r12_elctrcty,
      matches('^r13[a-ez]_.*'), 
      h = c(
...
(
  cv_r12vs13  <- section_r %>% 
    ...
    filter_at(vars(matches('^r13[a-ez]_.*'), -matches("fct$")), any_vars(!is.na(.))) %>% 
    select_cv(
      r12_elctrcty,
      matches('^r13[a-ez]_.*'), 
      h = c(
